...
class membership_point_member(osv.osv):
	...
	def action_activate(self, cr, uid, ids, context={}):
		...
		# update nama partner sesuai registrasi
			...
			obj_chjs = self.pool.get('membership.point.welcome.promo')
			reg_date = member.register_date																			# PROSI
			get_promo = obj_chjs.get_current(cr, uid, reg_date, context={'option':1})
			if get_promo:																							
				point_log = self.pool.get("membership.point.log")													
				values = {																							
					'type': 'welcome',																				
					'name': 'Welcome promo points',																	
					'member_id': member.id,																			
					'point_in': get_promo.welcome_point																
				}																									
				log_id = point_log.create(cr,uid,values)															
